    ...
    {/*text & image wrapper */}
      ...
      {/* text */ }
        <motion.div variants={fadeIn('right', 'tween', 0.2, 1.8)} className='flex-1 text-center lg:text-left'>
          <div className='pretitle text-zinc-600'>{pretitle}</div>
          <h2 className='h2 capitalize'>{title}</h2>
          <p className='mb-8 max-w-[560px]'>{subtitle}</p>
          <Link to='/aboutteam' className='btn hover:bg-zinc-600/70 text-black capitalize mx-auto lg:mx-0  hover:text-white lg:w-1/2 '>{btnText}</Link>
        </motion.div>
        {/* image */ }
        <motion.div 
